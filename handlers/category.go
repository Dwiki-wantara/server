...
import (
	categorry "dumbflix/dto/categori"
...
func (h *handlerCategory) FindCategori(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	categori, err := h.CategoryRepository.FindCategori()
	if err != nil {
	...
	response := dto.SuccessResult{Code: http.StatusOK, Data: categori}
...
func (h *handlerCategory) CreateCategory(w http.ResponseWriter, r *http.Request) {
	...
	request := new(categorry.CreateCategoryRequest)
...
func (h *handlerCategory) UpdateCategory(w http.ResponseWriter, r *http.Request) {
	...
	request := new(categorry.UpdateCategoryRequest)
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()}
		json.NewEncoder(w).Encode(response)
